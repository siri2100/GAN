...
class Generator(nn.Module):
    def __init__(self, dim_x, dim_z):
        ...
        self.dim_z = dim_z
        self.deconv1 = nn.ConvTranspose2d(dim_z, 8*dim_x, 4, 1, 0, bias=False)
        self.deconv2 = nn.ConvTranspose2d(8*dim_x, 4*dim_x, 4, 2, 1, bias=False)
        self.deconv3 = nn.ConvTranspose2d(4*dim_x, 2*dim_x, 4, 2, 1, bias=False)
        self.deconv4 = nn.ConvTranspose2d(2*dim_x, dim_x, 4, 2, 1, bias=False)
        self.deconv5 = nn.ConvTranspose2d(dim_x, 3, 4, 2, 1, bias=False)
        self.bn1 = nn.BatchNorm2d(8*dim_x)
        self.bn2 = nn.BatchNorm2d(4*dim_x)
        self.bn3 = nn.BatchNorm2d(2*dim_x)
        self.bn4 = nn.BatchNorm2d(dim_x)
...
class Discriminator(nn.Module):
    ...
    def forward(self, input, _):
        ...
        output = torch.sigmoid(self.conv5(x4))
        return output
